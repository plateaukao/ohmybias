...
### 變更
- **英文補空白只對空白鍵生效** — 「英文補空白」開啟時，只有無候選字按空白鍵直印英文才在尾端補空白；按 Enter 送出原碼維持原樣、不再補空白（0.5.0 起兩者都補）。

## [0.7.0] — 2026-08-26

### 新增
- **中／英切換快速鍵** — 設定 →「輸入」新增「中／英文切換」區塊：可另外指定一組快速鍵（例如 ⌃Space、⌥⇧E、F13）切換英文模式，也可以關掉「單擊 Shift 切換」。快速鍵可直接錄製，或從「常用組合」選單挑選（⌘Space、⌃Space 這類正被系統佔用的組合，錄製時會被系統先攔走，只能從選單挑）。快速鍵以 Carbon 全域 hot key 註冊（`RegisterEventHotKey`，無米蝦是目前輸入方式時才註冊、切走就取消），由系統在按鍵送到前景 app 之前攔下交給輸入法：iTerm2 這種不把 ⌘ 組合交給輸入法的 app 也切得動，Ghostty／Prowl 這種看輸入法沒插字就自己把按鍵重編碼送進 pty 的終端機也不會多出一個空白。（同一組合若被 Raycast、Alfred 之類的 app 也註冊了會被搶走，Carbon 偵測不到，設定頁只能提醒。）錄製時若該組合正被系統快速鍵佔用（Spotlight、切換輸入方式、指揮中心⋯ 這些由系統先攔截、輸入法收不到），會跳出對話框詢問是否代為停用系統那一組（寫入 `com.apple.symbolichotkeys` 並套用；若系統沒即時重讀會提示登出再登入）。沒有 ⌘⌃⌥ 的組合只接受 F 鍵，避免吃掉一般打字。
...
### 修正
- **⌘⇧／⌃⇧ 組合鍵放開後誤觸中／英切換** — 按 ⌘⇧S 之類的組合時，若先放 ⌘、再放 Shift，原本會把「Shift 單獨按住的那一小段」當成單擊而切換英文。現在 Shift 一旦與 ⌘⌃⌥ 並用，直到全部放開前都不再視為單擊。
